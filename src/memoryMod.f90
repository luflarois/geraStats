...
module memoryMod
    ...
    ! Tipo para armazenar os dados de uma combinação estação-variável
    type :: dat
     real, allocatable :: level(:)
     real, allocatable :: lat_min(:)
     real, allocatable :: lat_max(:)
     real, allocatable :: min_value(:)
     real, allocatable :: max_value(:)
    end type dat
    type(dat) :: data_stat(4,5)
    ...
    real, allocatable :: MinAtmosValues(:,:,:,:) 
    !# Min Value of atmospheric variables (nvar,nEst,nLat,nz)
    real, allocatable :: MaxAtmosValues(:,:,:,:) 
    !# Max Value of atmospheric variables (nvar,nEst,nLat,nz)

    integer :: monthCount
    !# Total of months present on data
    character(len=256), allocatable :: cams1FilesNames(:)
    real, allocatable :: atmosErrors(:,:,:,:,:) 
