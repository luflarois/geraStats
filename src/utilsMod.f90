...
module utilsMod
   ...
   ! --------------------------------------------------------------
   ! Função que retorna o número de dias entre dois anos,
   ! considerando anos bissextos.
   ! --------------------------------------------------------------
   function dias_entre_anos(ano_inicio, ano_fim) result(dias)
       implicit none
       integer, intent(in) :: ano_inicio, ano_fim
       integer :: dias
       integer :: ano
       ...
       ! Se o intervalo for invertido, chama recursivamente com sinal trocado
       if (ano_inicio > ano_fim) then
           dias = -dias_entre_anos(ano_fim, ano_inicio)
           return
       end if

       dias = 0
       do ano = ano_inicio, ano_fim - 1
           ! Verifica se o ano é bissexto (regra gregoriana)
           bissexto = (mod(ano, 400) == 0) .or. (mod(ano, 4) == 0 .and. mod(ano, 100) /= 0)
           if (bissexto) then
               dias = dias + 366
           else
